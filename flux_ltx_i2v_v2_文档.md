...
```
  ...
  flux1-dev (fp8)            clip_l + t5xxl_fp8             ae.safetensors
       │MODEL                   │CLIP                           │VAE
       ▼                        ├──────────┐                    │
  ② ModelSamplingFlux           ▼          ▼                    │
  max_shift=1.35          ⑤ CLIPText   ⑥ CLIPText              │
  base_shift=0.5          　EncodeFlux   Encode                  │
  960×544                  (正向Prompt) (负向Prompt)             │
       │MODEL               guidance=4.0    │                    │
       │                        │POS       │NEG                 │
       │                        │          ▼                    │
       │                        │    ⑦ ConditioningZeroOut      │
  ...
  │    ← negative (⑦)                          │
  │    ← latent_image (⑧)                      │
  │    dpmpp_2m + sgm_uniform, 28步            │
  │    CFG=1.0, seed=777, denoise=1.0          │
  └──────────────────┬─────────────────────────┘
                     │LATENT
                     ▼
  ┌──────────────────────────────────────────┐
  │  ⑩ VAEDecode  ← vae (④)                  │
  │  FLUX latent → 960×544 像素图             │
  └────────┬───────────────────┬─────────────┘
           │IMAGE               │IMAGE
           ▼                    ▼
  ┌──────────────┐    ╔═══════════════════════╗
  ...
  └──────────────┘         ▼
              ...
              ┌────────────▼────────────┬──────────────────┐
              │ ㉒ LTXVImgToVideo       │                  │
              │    ConditionOnly        │◄── ⑲ VAELoader   │
              │ strength = 0.85        │   LTX23_video_vae │
              │ bypass = false         │   _bf16 (1.4GB)   │
              │                        │◄── ㉑ EmptyLTXV    │
              │                        │   LatentVideo     │
              │                        │   960×544, 73帧   │
              └────────────┬───────────┘                   │
  ...
  │       │            │                                      │
  │       │            │  ⑯ UNETLoader                         │
  │       │            │  ltx-2.3-22b mxfp8 block32 (23GB)    │
  │       │            │       │MODEL                          │
  │       │            │       ▼                               │
  │       │            │  ㉝ LTXVApplySTG  ← 🆕 v2 新增!       │
  │       │            │  block_indices = "19"                 │
  ...
  │  │    ← guider (㉓)                                    │   │
  │  │    ← noise (㉖ RandomNoise, seed=888)               │   │
  │  │    ← sampler (㉕ KSamplerSelect, euler)             │   │
  │  │    ← sigmas (㉔ LTXVScheduler, 30步, terminal=0.05) │   │
  │  │    ← latent_image (㉒, 含首帧条件)                    │   │
  │  └──────────────────────┬─────────────────────────────┘   │
  │                         │LATENT                            │
  └─────────────────────────┼──────────────────────────────────┘
  ...
  │                         │         LTX 输出                  │
  │                         ▼                                   │
  │  ⑲ VAELoader ──→ ㉘ Set VAE Decoder Noise                  │
  │  LTX23_video     timestep=0.03, scale=0.015, seed=42       │
  │  _vae_bf16           │VAE                                   │
  │                      ▼                                      │
  │  ┌────────────────────────────────────┐                     │
  │  │ ㉙ LTXVTiledVAEDecode              │                     │
  ...
  │  ┌──────────────────────────────┐                           │
  │  │ ㉜ SaveVideo                  │                           │
  │  │ hypercar_flux_ltx_*.mp4      │                           │
  │  │ format=mp4, codec=h264       │                           │
  │  └──────────────────────────────┘                           │
  └─────────────────────────────────────────────────────────────┘

  输出: 960×544 MP4, 73帧, 24fps, ≈3秒, h264编码
```
